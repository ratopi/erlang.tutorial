%%
%% Example from http://www.erlang.org/article/11
%%
-module(wc).
...
gfile(File) ->
  ...
  {File, file_helper:with_file(File, Fun, {0, 0, 0})}.
...
output(Counts) ->
  io:format("~-25s ~-10s ~-10s ~-10s~n", ["file", "chars", "words", "lines"]),
  foreach(
    fun({File, {C, W, L}}) ->
      ok = io:format("~-25s ~-10w ~-10w ~-10w~n", [File, C, W, L])
    end,
    Counts).
